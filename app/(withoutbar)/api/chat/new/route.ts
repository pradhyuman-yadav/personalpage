...
export async function POST(request: NextRequest) {
  ...
  const { name, age, other_info } : {name: string, age: number, other_info: any} = reqBody;
  ...
  if (!name || !age) {
    ...
    .from('users')
    .insert([{ name, age, other_info }]).select().single();
  ...
  const { data: chatSessionData, error: chatSessionError } = await supabase
    .from('chat_sessions')
    .insert([{ user_id: userData.id, chat_id: chatId, current_agent: 'nurse' }]).select().single(); // Initialize current_agent
  ...
  if (chatSessionError) {
    ...
    const { error: initialMessageError } = await supabase.from('messages').insert([
        {
  ...
  return NextResponse.json({ chatId: chatSessionData.chat_id, userId: userData.id }, { status: 201 });
}