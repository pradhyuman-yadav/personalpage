...
const ChatWindow: React.FC<ChatWindowProps> = ({ chatId, initialMessages }) => {
  ...
  useEffect(() => {
    setIsInputDisabled(retryAfter !== null);
  }, [retryAfter]);
  ...
  const handleSendMessage = async (messageText: string) => {
    ...
    try {
      ...
      if (!historyResponse.ok) {
        throw new Error(
          `Failed to fetch chat history: ${historyResponse.status}`
        );
      }
      ...
      <div className="flex-grow overflow-y-auto">
        <div className="flex justify-between items-center p-4 border-b">
          ...
          <div className="text-sm text-gray-500">
            Chat ID: {chatId} | Name: {userName || "Loading..."} | Age:{" "}
            {userAge || "Loading..."}
          </div>
        </div>
            ...
            >
              {message.sender_type !== "user" &&
                message.sender_type !== "system" && (
                  <Avatar>
                    <AvatarImage
                      src={`/${message.sender_type}.png`}
                      alt={message.sender_type}
                    />
                    <AvatarFallback>
                      {message.sender_type.slice(0, 2).toUpperCase()}
                    </AvatarFallback>
                  </Avatar>
                )}
              <div
          ...
          {loading && (
            <div className="flex items-start gap-2 p-4">
              <Avatar>
                ...
                />
                <AvatarFallback>
                  {nextAgent ? nextAgent.slice(0, 2).toUpperCase() : "AG"}
                </AvatarFallback>
              </Avatar>
